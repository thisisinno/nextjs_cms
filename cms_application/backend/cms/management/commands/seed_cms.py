...
class Command(BaseCommand):
    ...
    def handle(self,*args,**kwargs):
        ...
        for i,title in enumerate(['Building Construction','Building Renew','Interior Design','Exterior Design','Building Renovation','Architectural Design']): Service.objects.get_or_create(slug=title.lower().replace(' ','-'),defaults={'title':title,'short_description':f'Professional {title.lower()} services for ambitious projects.','full_description':f'Our {title.lower()} team provides a disciplined, practical service tailored to your objectives.','category':'Construction','display_order':i})
